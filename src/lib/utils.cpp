...
bool checkFundamentalMatrix(const cv::Mat &h) {
    //find the determinant, if < 0, orientation non-preserving, orientation of points are lost
...
double crossRatio(const std::vector<cv::Point2f> &points) {
    ...
    return (euclideanDistance(points[0], points[2]) / euclideanDistance(points[0], points[3])) /
            (euclideanDistance(points[1], points[2]) / euclideanDistance(points[1], points[3]));
}
